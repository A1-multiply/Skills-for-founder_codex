...
Codex에서 한글 문서(`.hwp`, `.hwpx`, `.hwpml`)를 읽고, 분석하고, 수정하고, 양식까지 채울 수 있게 만든 스킬 패키지입니다.
...
| 기능 | 하는 일 | 사용할 때 |
| --- | --- | --- |
| 문서 읽기 | HWP/HWPX/HWPML 내용을 Markdown으로 추출 | 파일 내용이 무엇인지 알고 싶을 때 |
| 구조 분석 | JSON 블록, 제목, 메타데이터, 표, 컨트롤 구조 추출 | 문서 전체 구조나 표를 파악해야 할 때 |
...
| 결과 검증 | 수정된 파일을 다시 읽거나 렌더링해서 내용 반영 확인 | 편집 또는 양식 채우기 후 확인할 때 |
| 고급 디버그 | `rhwp`로 레이아웃, SVG, dump, 썸네일, 잠금 문서 확인 | 문서 레이아웃이나 저수준 구조를 봐야 할 때 |
...
| `skill/hwp-document-suite/scripts/hwp_fill_cells.mjs` | JSON 매핑으로 HWP 표 셀 여러 개 일괄 작성 |
...
```text
hwp-document-suite 써서 이 HWP 파일 전체 내용, 표, 이미지, 구조를 요약해줘.
```

문서 수정:

```text
hwp-document-suite 써서 이 HWP 문서의 2025를 전부 2026으로 바꾼 새 파일 만들어줘.
```

사업계획서 양식 채우기:

```text
hwp-document-suite 써서 창업 아이템 개요 요약표를 내 아이템에 맞게 개조식으로 타이트하게 채워줘.
```
